...
class Spk extends CI_Controller
{
    ...
    public function get_table_lamaran()
    {
        ...
        foreach ($resultss as $angga) {
            ...
            $row[] = '<a href="javascript:;"  class="btn btn-sm btn-block btn-primary" onClick="by_id(' . "'" . $angga->id_lamaran . "','detail'" . ')"><i class="fas fa fa-eye"></i> </a> <a href="javascript:;"  class="btn btn-sm btn-block btn-danger" onClick="by_id(' . "'" . $angga->id_lamaran . "','hapus'" . ')"><i class="fas fa fa-trash"></i> </a>';
            $data[] = $row;
        }
        $output = array(
            "draw" => $_POST['draw'],
    ...
    public function kirim_lamaran_saya()
    {
        ...
        if ($tingkat_pendidikan == 'SMA') {
            $edukasi_nilai = 0;
        } else if ($tingkat_pendidikan == 'D3 Sederajat') {
            $edukasi_nilai = 0.33;
        } else if ($tingkat_pendidikan == 'S1 Sederajat') {
            $edukasi_nilai = 0.67;
        } else {
            $edukasi_nilai = 1;
        }

        if ($lama_bekerja == '1 Tahun') {
            $experience_nilai = 0;
        } else if ($lama_bekerja == '2 Tahun') {
            $experience_nilai = 0.33;
        } else if ($lama_bekerja == '3 Tahun') {
            $experience_nilai = 0.67;
        } else {
            $experience_nilai = 1;
        }

        if ($nama_jabatan == 'Tidak Punya Jabatan') {
            $skills_nilai = 0;
        } else if ($nama_jabatan == 'Staff') {
            $skills_nilai = 0.2;
        } else if ($nama_jabatan == 'Ka.Seksi') {
            $skills_nilai = 0.4;
        } else if ($nama_jabatan == 'Ka.Bidang') {
            $skills_nilai = 0.6;
        } else if ($nama_jabatan == 'Ka.Subbag') {
            $skills_nilai = 0.8;
        } else {
            $skills_nilai = 1;
        }

        $education = $edukasi_nilai;
        $experience = $experience_nilai;
        $skills = $skills_nilai;
